...
import { Input, Button, Dropdown, Dialog, DropdownItem, DropdownMenu, Popover, Checkbox, Tooltip, Upload, Autocomplete, Notification } from 'element-ui';
...
import Penaltytype from '../../classes/penaltytype';
...
export default class EldComponent extends Vue {
    ...
    personrelativeFullbirthlocation: string;
    ...
    personpenaltyPenaltytype: number;
    ...
    data() {
        return {
            ...
            personrelativeFullbirthlocation: "",
    ...
    mounted() {
        ...
        this.filteredSubjects = this.subjects.filter(option => (option.category == 11 || option.category == 10 || option.category == 9));
    }
    ...
    selectPerson(id: number, autoupdate: boolean = false) {
        console.log(this.rightToDocuments);
        //this.rightToDocuments = true;
    ...
    addPersonrelative() {
        ...
        fetch('/api/Personrelative', {
            ...
            body: JSON.stringify(<Personrelative>{
                ...
                birthlocation: this.personrelativeFullbirthlocation,
    ...
    addPersonrelativeButton(person: Person) {
        ...
        this.personrelativeFullbirthlocation = "";
    ...
    updatePersonrelativeButton(person: Person, personrelative: Personrelative) {
        ...
        this.personrelativeFullbirthlocation = this.personrelativeMenuelement.birthlocation;
    ...
    completePersonrelativeButton(person: Person) {
        if (this.personrelativeMenuelement == null) {
            ...
            this.personrelativeMenuelement.birthlocation = this.personrelativeFullbirthlocation;
    ...
    addPersonpenalty() {
        if(this.penalty.isspoken == 1 && this.isspoken == true){
            this.typepenalty = 1;
        }else{
            this.typepenalty = 0;
        }
        ...
        fetch('/api/Personpenalty', {
            ...
            body: JSON.stringify(<Personpenalty>{
                ...
                penaltytype: this.prepareNumToExport(this.personpenaltyPenaltytype),
    ...
    updatePersonpenalty(person: Person, personpenalty: Personpenalty) {
        if(this.penalty.isspoken == 1 && this.isspoken == true){
            personpenalty.isspoken = 1;
        }else{
            personpenalty.isspoken = 0;
        }
        this.prepareToExport(person);
        fetch('/api/Personpenalty', {
            method: 'post',
    ...
    getPenaltytype(penaltytypeid: number): string {
        if (penaltytypeid == null || penaltytypeid == 0) {
            return "";
        }
        let penaltytype: Penaltytype = this.penaltytypes.find(t => t.id == penaltytypeid);
        if (penaltytype != null) {
    ...
    updatePersonpenaltyButton(person: Person, personpenalty: Personpenalty) {
        ...
        this.personpenaltyPenaltytype = this.personpenaltyMenuelement.penaltytype;
    ...
    completePersonpenaltyButton(person: Person) {
        if (this.personpenaltyMenuelement == null) {
            ...
            this.personpenaltyMenuelement.penaltytype = this.prepareNumToExport(this.personpenaltyPenaltytype);
    ...
    addPersoneducation() {
        ...
        fetch('/api/Personeducation', {
            ...
            body: JSON.stringify(<Personeducation>{
                ...
                name3: this.personeducationName3,
    ...
    updatePersoneducationButton(person: Person, personeducation: Personeducation) {
        ...
        this.personeducationName3 = this.personeducationMenuelement.name3;
    ...
    completePersoneducationButton(person: Person) {
        ...
        if (this.personeducationMenuelement == null) {
            ...
            this.personeducationMenuelement.name3 = this.personeducationName3;
    ...
    createPersonReference(person: Person) {
        let idDoc: number = 1;
        this.prepareToExport(person);
        fetch('/api/Person/PersonReference/' + idDoc, {
            ...
            .then(x => {
                if (x.ok) {
                    x.data.then(time => download(time));
                } else {
                    this.$notify.error({
                        title: "Ошибка",
                        message: "Документ не сформирован, повторите попытку",
                        offset: 100
                    });
            }
        })
    ...
    downloadExcel() {
        ...
        fetch('/api/Person/PersonReference', {
            method: 'post',
            body: JSON.stringify(<Person>{
            }),
            credentials: 'include',
    ...
    filterMethod(query) {
        ...
        this.filteredSubjects = this.subjects.filter(option => {
            return (option.category == 9) && option.name1.toLowerCase().startsWith(query.toLowerCase());
        }).slice(0, 20);
    ...
    setPenalty(penalty: number){
        this.penalty = this.penalties.find(el => el.id == penalty);
    }

    setPenaltytype(penaltytype: number){
        this.penaltytype = this.penaltytypes.find(el => el.id == penaltytype);
    }
}